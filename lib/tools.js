...
exports.init_room = function(room){
	for (var mem in room.member){
		...
		var send_info = {
			...
			data: {
				room: room.id,
				name: their_name,
				text: 'find people, left 10 second into fight!!'
			}
		}

		setTimeout(function(){
			room.member[mem].connection.send(JSON.stringify());
		}, 10000)
		...
		// init answer order
		init_fight_order(room);

		// perpare question to send client
		// where is redis??
	}
}
...
function init_fight_order(room){
