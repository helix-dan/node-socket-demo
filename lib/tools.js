...
exports.leave_fight = function(user_id){
	if (typeof fight_users[user_id] === "undefined"){
		...
		fight_rooms[the_room_id].timer && clearTimeout(fight_rooms[the_room_id].timer);
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// there have !!!!!!!!
// console.log(fight_users[user_id].wrong_num)
	// answer is right
...
// console.log('@@@@@ end the fight @@@@@')
	...
	// delete the room
	delete fight_rooms[the_room_id]
// console.log("iiiiiii delllllllllll allllllll")
// console.dir(fight_users);
}

function battle(room_id){
// console.log('into room and battle')
	var room = fight_rooms[room_id];
...
function send_question(question, room_id){
	// clear the time out
	fight_rooms[room_id].timer && clearTimeout(fight_rooms[room_id].timer);
...
console.log('########');
console.dir(send_info.data['info']);
	...
	room = set_question_timeout(room, 19000);
	fight_rooms[room.id] = room;
}

function set_question_timeout(room, time){
console.log("$$$$$$$$ create a timer $$$$$$$$")
...
console.log('############ diao yong le timer #############')
			fight_users[room.member[i]].connection.send(JSON.stringify(send_info));
		}

		var out_user_id = fight_users[room.answer_user_id].fight_with

// console.log(out_user_id + ' wrong:')
// console.log(fight_users[out_user_id].wrong_num);
