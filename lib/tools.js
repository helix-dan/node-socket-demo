...
function getQuestion(level, category, room, client){
	// get it for redis 
	var allQuestionNumber = Math.floor(Math.random()*130) + 1;
		...
		if(err){
			...
			if(room.type === 'AI' && fightUsers[room.answerUserId].type === 'AI'){
				var aiTimer = setTimeout(function(){
					...
					var go_on = computeThePoint(room.answerUserId, (Math.floor(Math.random()*3)+1).toString());
					if(go_on){
						// go on battle
						goOnTheBattle(room.answerUserId, client);
					} else {
						// end the battle
						endTheFight(room.answerUserId);
					}
				}, (Math.floor(Math.random()*8) + 7)*1000)
